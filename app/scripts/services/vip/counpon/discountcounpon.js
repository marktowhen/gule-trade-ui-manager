...
shopbackApp.service('DiscountcounponService', function ($http, $location, ApiService) {
	...
	this.list = function(discountcounpon, from, size){
		return $http.get(ApiService.api.counpon.discountcounpon.list.replace(':from', from).replace(':size', size)
			,{params:{"cardNum": discountcounpon.cardNum , "value":discountcounpon.value ,"locked": discountcounpon.locked,'cardNumStart': discountcounpon.cardNumStart,'cardNumEnd':discountcounpon.cardNumEnd }});
	}
	//解锁
	this.unlock = function(ids){
		return $http.put(ApiService.api.counpon.discountcounpon.unlock, ids);
	}
